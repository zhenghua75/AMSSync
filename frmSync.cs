...
//			strDBClientCon="server=127.0.0.1;uid=sa;pwd=sa;database=AMSCM_DongLai";//测试用
		...
		private void RealTimeSync()
		{
			...
			try
			{
				...
				strSql="select * from tbAssociatorSync order by vcCardID,iSerial";
				adp=new SqlDataAdapter(strSql,conClient);
				dttmp=new DataTable();
				adp.Fill(dttmp);
				if(dttmp.Rows.Count>0)
				{
					foreach(DataRow dr in dttmp.Rows)
					{
						...
						if(drr[0].ToString()=="1")
						{
							...
							drr.Close();
							strSql="update tbAssociator set vcAssType='"+dr["vcAssType"].ToString()+"',vcAssState='"+dr["vcAssState"].ToString()+"',nCharge="+dr["nCharge"].ToString()+",iIgValue="+dr["iIgValue"].ToString()+",dtOperDate='"+dr["dtOperDate"].ToString()+"',vcCardSerial='"+dr["vcCardSerial"].ToString()+"' where vcCardID='"+dr["vcCardID"].ToString()+"';";
